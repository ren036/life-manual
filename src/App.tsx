...
export default function App() {
  ...
  async function saveItem(item: SavedItem, file?: File) {
    if (addMode === 'recipes') {
      await insertRecipe(item as Recipe, file);
      setRecipes((current) => [item as Recipe, ...current]);
    } else if (addMode === 'documents') {
      await insertDocument(item as DocumentItem, file);
  ...
  async function openRecipe(item: Recipe) {
    if (item.hasFile && await openAttachment(item.id)) return;
    setToast(item.notes || item.ingredients.join('、') || '暂无更多说明');
  }

  async function openDocument(item: DocumentItem) {
    if (item.hasFile && await openAttachment(item.id)) return;
    setToast(item.description || '这份资料还没有附件');
  }
  ...
  useEffect(() => {
    ...
    context.registerTool({
      name: 'create_task',
      title: '新建待办',
      description: '在生活手册中添加一件要做的事。',
      inputSchema: {
        type: 'object',
        properties: { title: { type: 'string' }, notes: { type: 'string' }, category: { type: 'string' } },
        required: ['title'],
        additionalProperties: false,
      },
      annotations: { readOnlyHint: false, untrustedContentHint: false },
      async execute(input: { title: string; notes?: string; category?: string }) {
        const item: TaskItem = {
          id: crypto.randomUUID(),
          title: input.title.trim(),
          notes: input.notes?.trim() || '',
          category: input.category?.trim() || '生活',
          priority: '普通',
          completed: false,
          createdAt: Date.now(),
        };
  ...
  return (
    <main className="shell">
      <div className="app-frame">
        <header className="topbar"><div><span>我的生活</span><h1>{pageTitles[tab]}</h1></div></header>
        <div className="content-area">
          {loading && <p className="empty-state">正在整理你的记录…</p>}
          {!loading && tab === 'home' && <HomePage recipes={recipes} documents={documents} tasks={tasks} onNavigate={setTab} onAdd={startAdd} />}
          {!loading && tab === 'recipes' && <RecipesPage recipes={recipes} onOpen={openRecipe} />}
          {!loading && tab === 'documents' && <DocumentsPage documents={documents} onOpen={openDocument} />}
          {!loading && tab === 'tasks' && <TasksPage tasks={tasks} onToggle={toggleTask} />}
        </div>
        {tab !== 'home' && <button className="floating-add" onClick={() => startAdd(floatingMode)} aria-label="新增"><Plus /></button>}
        <BottomNav active={tab} onChange={setTab} />
        {toast && <div className="toast" aria-live="polite">{toast}</div>}
      </div>
      <AddRecordSheet open={sheetOpen} mode={addMode} onClose={() => setSheetOpen(false)} onSave={saveItem} />
    </main>
  );
}
